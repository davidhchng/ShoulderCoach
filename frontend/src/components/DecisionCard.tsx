...
const tile = "glass soft-pop border border-white/6 px-5 py-5";

export default function DecisionCard({ result }: DecisionCardProps) {
  if (result.insufficient_data) {
    return (
      <div className={`${tile} text-center`}>
        <p className="text-xs uppercase tracking-widest text-gray-500 mb-2">Insufficient Data</p>
  ...
  return (
    <div className="space-y-px">
      {result.low_sample_warning && (
        <div className="glass soft-pop border border-yellow-800/40 px-4 py-2 bg-yellow-950/30">
      ...
      {/* VERDICT */}
      <div className={tile}>
        <p className="text-[9px] uppercase tracking-widest text-gray-500 mb-2">Call</p>
        ...
        <span className={`text-[9px] font-mono uppercase tracking-widest px-2 py-0.5 border ${
          result.confidence === "high"
            ...
            ? "border-yellow-700/60 text-yellow-500"
      ...
      {result.decision_type === "three_vs_two" &&
        ...
        <div className="glass soft-pop border border-white/6">
          <div className="px-5 pt-4 pb-2">
            <p className="text-[9px] uppercase tracking-widest text-gray-500">Decision Math</p>
          </div>
          <ProbabilityTree
      ...
      {/* HEAD-TO-HEAD or single stat */}
      {hasComparison ? (
        <div className={tile}>
          <p className="text-[9px] uppercase tracking-widest text-gray-500 mb-4">Head to Head</p>
          <CompareBar
          ...
          <div className="flex justify-between mt-3">
            <p className="text-[9px] text-gray-500">n={result.primary_sample_size} <ConfidenceBadge n={result.primary_sample_size} /></p>
            {result.comparison_sample_size != null && (
              <p className="text-[9px] text-gray-500">n={result.comparison_sample_size} <ConfidenceBadge n={result.comparison_sample_size} /></p>
            )}
          </div>
        </div>
      ) : (
        <div className={`${tile} flex items-center gap-5`}>
          {showArc && (
            <div className="relative flex-shrink-0">
              <StatArc value={primaryDisplay} color={color} size={96} />
              <div className="absolute inset-0 flex flex-col items-center justify-center">
                <span className="text-lg font-black tabular-nums leading-none text-white">{primaryDisplay.toFixed(1)}</span>
                <span className="text-[8px] text-gray-500 uppercase tracking-wide">pct</span>
              </div>
            </div>
          )}
          <div className="flex-1 min-w-0">
            <p className="text-[9px] uppercase tracking-widest text-gray-500 mb-0.5">{result.primary_stat_label}</p>
            {!showArc && (
              <p className="text-2xl font-black tabular-nums text-white">{formatVal(result.primary_stat, result.primary_stat_label)}</p>
            )}
            <p className="text-[9px] text-gray-500 mt-1">n={result.primary_sample_size} <ConfidenceBadge n={result.primary_sample_size} /></p>
          </div>
        </div>
      )}
      ...
      {result.edge_pct !== null && result.edge_pct !== undefined && result.edge_pct > 0 && (
        <div className={`${tile} flex items-center gap-3`}>
          ...
          <div>
            <p className="text-[9px] uppercase tracking-widest text-gray-500">Edge</p>
            <p className="text-sm font-black text-white">
              <span className="text-orange-400">+{result.edge_pct}{unit}</span> over the alternative
            </p>
          </div>
        </div>
      )}
      ...
      {result.details && Object.keys(result.details).filter(k => result.details[k] != null).length > 0 && (
        <div className={tile}>
          <p className="text-[9px] uppercase tracking-widest text-gray-500 mb-3">Details</p>
          <div className="space-y-2">
            {Object.entries(result.details).map(([key, val]) => {
              if (val === null || val === undefined) return null;
              return (
                <div key={key} className="flex justify-between items-baseline">
                  <span className="text-[9px] uppercase tracking-widest text-gray-500">{key.replace(/_/g, " ")}</span>
      ...
      {result.narrative && (
        <div className={tile}>
          <p className="text-[9px] uppercase tracking-widest text-gray-500 mb-2">Analysis</p>
          <p className="text-gray-200 text-sm leading-relaxed">{result.narrative}</p>
          {!result.narrative_available && (
            <p className="text-gray-600 text-[9px] mt-2 uppercase tracking-widest">stats only</p>
          )}
        </div>
      )}
    </div>
